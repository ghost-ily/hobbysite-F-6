...
class CommissionListView(ListView):
    ...
    def get_queryset(self):
        return Commission.objects.select_related('author', 'author__user').prefetch_related('jobs', 'jobs__applications').annotate(
            status_order=Case(
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            ...
            applied_job_ids = list(
                JobApplication.objects.filter(applicant=profile).values_list('job_id', flat=True)
            )
